...
import type { Kundenstamm, Monteure, Terminverwaltung, CreateKundenstamm, CreateMonteure, CreateTerminverwaltung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getKundenstamm(): Promise<Kundenstamm[]> {
    ...
    return enrichLookupFields(records, 'kundenstamm');
  }
  static async getKundenstammEntry(id: string): Promise<Kundenstamm | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Kundenstamm;
    return enrichLookupFields([record], 'kundenstamm')[0];
  }
  static async createKundenstammEntry(fields: CreateKundenstamm) {
    return callApi('POST', `/apps/${APP_IDS.KUNDENSTAMM}/records`, { fields: cleanFieldsForApi(fields as any, 'kundenstamm') });
  }
  static async updateKundenstammEntry(id: string, fields: Partial<CreateKundenstamm>) {
    return callApi('PATCH', `/apps/${APP_IDS.KUNDENSTAMM}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kundenstamm') });
  }
  ...
  // --- MONTEURE ---
  static async getMonteure(): Promise<Monteure[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MONTEURE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Monteure[];
    return enrichLookupFields(records, 'monteure');
  }
  static async getMonteureEntry(id: string): Promise<Monteure | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MONTEURE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Monteure;
    return enrichLookupFields([record], 'monteure')[0];
  }
  static async createMonteureEntry(fields: CreateMonteure) {
    return callApi('POST', `/apps/${APP_IDS.MONTEURE}/records`, { fields: cleanFieldsForApi(fields as any, 'monteure') });
  }
  static async updateMonteureEntry(id: string, fields: Partial<CreateMonteure>) {
    return callApi('PATCH', `/apps/${APP_IDS.MONTEURE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'monteure') });
  }
  static async deleteMonteureEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MONTEURE}/records/${id}`);
  }
