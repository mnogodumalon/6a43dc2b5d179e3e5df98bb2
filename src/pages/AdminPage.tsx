...
import { KundenstammViewDialog } from '@/components/dialogs/KundenstammViewDialog';
...
const MONTEURE_FIELDS = [
  ...
  { key: 'monteur_telefon', label: 'Telefon', type: 'string/tel' },
  { key: 'monteur_email', label: 'E-Mail', type: 'string/email' },
  { key: 'monteur_notizen', label: 'Notizen', type: 'string/textarea' },
];
const TERMINVERWALTUNG_FIELDS = [
  ...
  { key: 'beschreibung', label: 'Beschreibung der Aufgabe', type: 'string/textarea' },
  { key: 'kunde', label: 'Kunde', type: 'applookup/select', targetEntity: 'kundenstamm', targetAppId: 'KUNDENSTAMM', displayField: 'vorname' },
  { key: 'monteur', label: 'Monteur', type: 'applookup/select', targetEntity: 'monteure', targetAppId: 'MONTEURE', displayField: 'monteur_vorname' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'offen', label: 'Offen' }, { key: 'erledigt', label: 'Erledigt' }, { key: 'verschoben', label: 'Verschoben' }] },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'kundenstamm', label: 'Kundenstamm', pascal: 'Kundenstamm' },
  { key: 'monteure', label: 'Monteure', pascal: 'Monteure' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kundenstamm');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenstamm': return (data as any).kundenstamm as Kundenstamm[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'terminverwaltung':
        lists.kundenstammList = (data as any).kundenstamm ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'terminverwaltung' && fieldKey === 'kunde') {
      const match = (lists.kundenstammList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenstamm': return KUNDENSTAMM_FIELDS;
      case 'monteure': return MONTEURE_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kundenstamm': return {
        create: (fields: any) => LivingAppsService.createKundenstammEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKundenstammEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKundenstammEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kundenstamm' || dialogState?.entity === 'kundenstamm') && (
        <KundenstammDialog
          open={createEntity === 'kundenstamm' || dialogState?.entity === 'kundenstamm'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kundenstamm' ? handleUpdate : (fields: any) => handleCreate('kundenstamm', fields)}
          defaultValues={dialogState?.entity === 'kundenstamm' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'terminverwaltung' || dialogState?.entity === 'terminverwaltung') && (
        <TerminverwaltungDialog
          ...
          kundenstammList={(data as any).kundenstamm ?? []}
      ...
      {viewState?.entity === 'kundenstamm' && (
        <KundenstammViewDialog
          open={viewState?.entity === 'kundenstamm'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kundenstamm', record: r }); }}
        />
      )}
