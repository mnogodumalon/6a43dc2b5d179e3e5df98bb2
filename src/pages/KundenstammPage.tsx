...
export default function KundenstammPage() {
  ...
  const [records, setRecords] = useState<Kundenstamm[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Kundenstamm | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Kundenstamm | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getKundenstamm());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Kundenstamm['fields']) {
    await LivingAppsService.createKundenstammEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Kundenstamm['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateKundenstammEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteKundenstammEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Kundenstamm"
      subtitle={`${records.length} Kundenstamm im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Kundenstamm suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/kundenstamm/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Kundenstamm. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
