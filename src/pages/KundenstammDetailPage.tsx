...
import { formEnhancements } from '@/config/form-enhancements/Kundenstamm';
import { evalComputed } from '@/config/form-enhancements/types';

export default function KundenstammDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Kundenstamm | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getKundenstamm();
  ...
  async function handleUpdate(fields: Kundenstamm['fields']) {
    if (!record) return;
    await LivingAppsService.updateKundenstammEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteKundenstammEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/kundenstamm');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/kundenstamm')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/kundenstamm')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.vorname ?? 'Kundenstamm'} />
