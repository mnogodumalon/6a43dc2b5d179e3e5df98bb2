...
import type { Kundenstamm } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kundenstamm';
...
interface KundenstammDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Kundenstamm['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Kundenstamm['fields'];
...
export function KundenstammDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenstammDialogProps) {
  const [fields, setFields] = useState<Partial<Kundenstamm['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Kundenstamm['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kundenstamm');
      await onSubmit(clean as Kundenstamm['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Kundenstamm['fields']>;
      });
      if (gps) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Kundenstamm bearbeiten' : 'Kundenstamm hinzufügen';
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "standort": "Standort auf der Karte", "notizen": "Notizen"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KUNDENSTAMM} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
