...
import type { Terminverwaltung, Kundenstamm, Monteure, LookupValue } from '@/types/app';
...
interface TerminverwaltungDialogProps {
  ...
  defaultValues?: Omit<Terminverwaltung['fields'], 'auftragsart' | 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  ...
  status: LOOKUP_OPTIONS['terminverwaltung']?.['status'] ?? [],
};
...
export function TerminverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenstammList, monteureList, enablePhotoScan = true, enablePhotoLocation = true }: TerminverwaltungDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">Beschreibung der Aufgabe</Label>
        <Textarea
          id="beschreibung"
          placeholder="Was muss gemacht werden? Geräte, Symptome, Wünsche..."
    ...
    'kunde': (
      <div key="kunde" className="space-y-1.5">
        ...
        <Combobox
          ...
          createLabel="Neu in Kundenstamm"
        />
      </div>
    ),
    'monteur': (
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'verschoben'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'verschoben' ? undefined : 'verschoben') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'verschoben'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Zusätzliche Informationen, Zufahrtshinweise..."
  ...
  const FIELD_LABELS: Record<string, string> = {"termin_datum": "Datum und Uhrzeit", "auftragsart": "Auftragsart", "beschreibung": "Beschreibung der Aufgabe", "kunde": "Kunde", "monteur": "Monteur", "status": "Status", "bemerkungen": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"vorname": "Vorname", "nachname": "Nachname", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "standort": "Standort auf der Karte", "notizen": "Notizen"}, "monteur": {"monteur_vorname": "Vorname", "monteur_nachname": "Nachname", "monteur_telefon": "Telefon", "monteur_email": "E-Mail", "monteur_notizen": "Notizen"}};
    ...
    {createKundenstammOpen && (
      <KundenstammDialog
        ...
        defaultValues={createKundenstammInitial
          ? ({ vorname: createKundenstammInitial } as any)
          : undefined}
      />
    )}
    {createMonteureOpen && (
