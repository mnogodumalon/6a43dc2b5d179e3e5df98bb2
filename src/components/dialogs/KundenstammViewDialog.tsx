...
interface KundenstammViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Kundenstamm | null;
  onEdit: (record: Kundenstamm) => void;
}

export function KundenstammViewDialog({ open, onClose, record, onEdit }: KundenstammViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Kundenstamm anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="pt-2 border-t border-border">
            <AttachmentsSection appId={APP_IDS.KUNDENSTAMM} recordId={record.record_id} readOnly />
          </div>
        </div>
      </DialogContent>
    </Dialog>
